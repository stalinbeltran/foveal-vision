#!/usr/bin/env python3
"""La red de ESTE experimento: UN kernel 5×5 y `padding=0` — SIN relleno.
...
QUE PASA CON EL TAMANO
   Con k=5 y sin relleno, un mapa 20x20 sale 16x16. Con UN canal, la cabeza
   recibe 1 x 16 x 16 = 256 features (400 con relleno).

   ⚠⚠ OJO: son MAS features que las 196 del gemelo de 7x7, no menos. Un kernel
   mas pequeno RECORTA MENOS. Asi que este experimento mueve DOS cosas a la vez
   y en sentidos opuestos --+31 % de features y la mitad de parametros en L1--,
   que es justo lo que le permite decidir cual de las dos manda. El criterio,
...
RED_BASE = REPO / "configs" / "networks" / "plana-20-1k5.yaml"
...
def _comprobar() -> int:
    ...
    print("  ⚠ y la cabeza CRECE respecto al gemelo de 7x7: 256 features contra 196,")
    print("    porque un kernel mas pequeno recorta menos. Dos ejes a la vez.")
    return 0
