#!/usr/bin/env python3
"""La red de ESTE experimento: UN kernel 7×7 y `padding=0` — SIN relleno.
...
QUE PASA CON EL TAMANO
   Con k=7 y sin relleno, un mapa 20x20 sale 14x14. Con UN canal, la cabeza
   recibe 1 x 14 x 14 = 196 features (400 con relleno). O sea que esta red NO es
   «igual salvo el relleno»: es mas pequena. Es inevitable --no rellenar
   encoge-- y hay que tenerlo delante al comparar.

   ⚠ Y 196 es la MITAD EXACTA de los 392 del gemelo de 2 kernels, que es todo el
   punto de este experimento: contrasta la tendencia de ~0,09 de f1 por cada
   mitad de features SIN volver a mover el relleno. El criterio, congelado antes
   de correr nada, en `../instrucciones/02-criterio.md`.
...
RED_BASE = REPO / "configs" / "networks" / "plana-20-1k7.yaml"
...
def _comprobar() -> int:
    ...
    print("  ⚠ y la cabeza ENCOGE: es inevitable al no rellenar. 196 features,")
    print("    la MITAD EXACTA de los 392 del gemelo de 2 kernels.")
    return 0
