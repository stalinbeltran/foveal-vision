#!/usr/bin/env python3
"""La red de ESTE experimento: 2 kernels 7×7 y `padding=0` — SIN relleno.
...
QUE CAMBIA, Y POR QUE NO CABIA EN LA CONFIG
   ...
   relleno». Comprobado en `--comprobar`: los kernels salen identicos bit a bit a
   los del gemelo con relleno.

QUE PASA CON EL TAMANO
   Con k=7 y sin relleno, un mapa 20x20 sale 14x14. La cabeza pasa de 800 a 392
   features (2 canales x 14 x 14). O sea que esta red NO es «igual salvo el
   relleno»: es mas pequena. Es inevitable --no rellenar encoge-- y hay que
   tenerlo delante al comparar: parte de lo que se mida sera eso.
...
RED_BASE = REPO / "configs" / "networks" / "plana-20-2k7.yaml"
...
class PlanaSinPadding(FoveatedRegionalNN):
    ...
    Hereda en vez de copiar los 300 lineas del builder: lo que este experimento
    prueba es UNA cosa --quitar el relleno-- y reescribir el resto seria abrir la
    puerta a que divergiera del gemelo por accidente.
    """
...
def _comprobar() -> int:
    ...
    assert torch.equal(cv.weight, cvc.weight), "los kernels NO son los mismos"
    ...
    print("  ✓ mismos kernels bit a bit · padding 0 · la salida sigue siendo 4x3 = 12")
    print("  ⚠ y la cabeza ENCOGE: es inevitable al no rellenar, y hay que tenerlo")
    print("    delante al comparar con el gemelo.")
    return 0
