...
import {
  CornerDots, CornerFilter, CornerFilterAviso,
} from "../components/CornerDots";
    ...
    <div>
      ...
      <div className="row">
        <div className="card">
          {result ? (
            <div style={{ position: "relative", display: "inline-block",
              ...
              <svg viewBox={`0 0 ${W} ${H}`}
                ...
                <CornerDots
                  corners={show.corners ? result.corners : []}
                  raw={show.raw ? result.raw : []}
                  windowSize={result.knobs?.window_size ?? 16}
                  hidden={ocultas} width={W} height={H} />
          ...
          <div style={{ marginTop: 8 }} data-testid="predict-corners">
            <CornerFilter order={ranuras} hidden={ocultas} onToggle={alternar}
              onAll={() => setOcultas([])}
              counts={Object.fromEntries(ranuras.map((c) => [c,
                (result?.corners ?? []).filter((d: any) => d.corner === c).length]))} />
          </div>
