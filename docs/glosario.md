...
| ⚠ **borde de la IMAGEN (`edge`) ≠ borde difuso (`border`)** | **Colisión nueva (2026-08-31), y el español la fabrica: dice «borde» para las dos.** `border_px` es el **ancho del anillo** de contexto de la vista; `edge` es que **se acaba la imagen**. En código nunca se mezclan: `border_*` para el anillo, `edge_*` para el límite de la página. Leerlas como una sola hace que `border_px: 0` parezca «no hay borde de imagen», que es falso para **todas** las ventanas de un control plano |
| **`edge_inputs`** | Campo de C: `off` · `pad` · `dist`. Cuatro escalares por ventana (orden `EDGE_SIDES` = `L, T, R, B`) que van **directos a la cabeza**, sin pasar por ninguna convolución, diciendo si la imagen se acaba por cada lado. `0` = no hay borde por ahí, `1` = está aquí, **en los dos modos**. `off` es el default y no añade ninguna entrada. instructionsNewNN.md §6bis |
