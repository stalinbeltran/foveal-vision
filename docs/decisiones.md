...
| **F11** | **¿Se regenera el dato para que la métrica de tarea pueda decidir?** Medido (2026-07-26): sd del F1 de párrafo entre imágenes = 0,372 sobre **20 imágenes de val** → **±0,083** por run, más ruido que las diferencias a distinguir. Llegar a ±0,026 pide ~200 imágenes de val (~2000 en total, lo que protocolo.md §3 ya pedía) | **No la tomo yo.** Regenerar **invalida la comparabilidad** con los 130 runs, 4 recorridos y 4 estudios actuales (otro fingerprint de B). Alternativa: seguir con 20 y usar la métrica de tarea **solo como informe del ganador**, nunca para elegir entre puntos. Detalle y aritmética en [metrica-de-tarea.md](metrica-de-tarea.md) §4 | 9 |
| **F12** | **¿Qué es exactamente «la CNN plana de coste equivalente»** del primer experimento (protocolo.md §6)? Hoy **no se puede construir**: sin periferia la geometría se rechaza con `no_periphery`, y `d=1` sigue siendo dos ramas enmascaradas | Aplazada hasta que exista la métrica de tarea. Cuando se abra: definir si el control es una arquitectura nueva en un registro (`arch`) o una degeneración permitida de la foveada, y **escribir el criterio antes de medir** | investigación |
