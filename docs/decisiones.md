...
| **F7** | ✅ **CERRADA A MEDIAS (2026-08-31), por el lado barato.** La mitad que C11 dejaba abierta —«un canal de máscara como entrada de la red»— se resuelve **sin canal**: `edge_inputs` mete **4 escalares por lado** (`fv.fovea.EDGE_SIDES`) **directos a la cabeza**, saltándose las convoluciones. Decidido así por el usuario y por dos razones que están en instructionsNewNN.md §6bis: la señal **no es espacial** (como canal sería una constante pintada en N×N celdas) y las ramas están **enmascaradas por región**, así que una señal por el input sería invisible para la rama central, que es la que predice las esquinas. Cuesta **+48 pesos** (+0,03 %, medido) contra el canal, que ensancharía las dos primeras convs. ⚠ **Lo que NO cierra**: la máscara de cobertura sigue **sin entrar** en la red en ninguna forma (F0 la enseña para depurar y nada más), y **`edge_inputs` no está barrido** — que exista y entrene no es que mejore. *(La pregunta original se conserva a continuación.)* **Relleno de bordes de imagen en la vista: ¿valor del relleno + máscara, o solo máscara?** | Media enmascarada (el tono del relleno no entra en el número) + máscara de cobertura — la lección medida del hermano | 3 |
...
| C15 | **El borde de la IMAGEN entra por la CABEZA, no por el input** (2026-08-31, resuelve la mitad de F7 que C11 dejó abierta): `edge_inputs` ∈ {`off`·`pad`·`dist`} añade 4 escalares por lado concatenados a las features justo antes de la `Linear`, **fuera del ReLU y fuera del dropout**. `off` es el default y es **bit-idéntico** a la red anterior (los checkpoints cargan `strict`). El canal de máscara N×N **no se construye**: no es una señal espacial, y las ramas están enmascaradas por región | `fv/fovea.edge_features`, `fv/models/builder`, instructionsNewNN.md §6bis, `tests/test_edge_inputs.py` |
