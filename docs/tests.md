...
| ⑬b | la excepción declarada de ⑬ (`fv.fallidos`) | la verdad **recompuesta desde B es la misma que la de A**, caja a caja, en un mundo donde existen las dos — sin esto, `--verdad ventanas` es una creencia; un párrafo cortado por el borde se **marca** (`gt_completa: false`) y **no se inventa** media caja; y **sin la fuente NO se cae solo** al camino degradado: falla con `verdad_necesita_fuente` (con el `source_not_found` real del loader, borrando la fuente, no simulándolo). Lo que sale es un B entrenable —`fv-train` corre sobre él—, con huella propia, `sample_idx` sin renumerar, escrito **atómicamente** (un corte no quema el nombre) y con la procedencia y el origen de la verdad en el manifest. En `tests/test_fallidos.py` | (dataset de fallos) |
