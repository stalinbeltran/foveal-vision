...
| **`dropout`** | Apaga al azar una fracción de las neuronas en cada paso de entrenamiento, para que la red no se apoye siempre en las mismas y **memorice** el dataset. Es el mando de **regularización** de la red, el hermano de `weight_decay` (que regulariza desde la receta). 0,0 = apagado. | **0,0** (no implementado) | **0,0** (no implementado) | **NUNCA — y hoy NO ES BARRIBLE** | ⚠⚠ **El parámetro está en la documentación pero NO en el código.** Lo nombran [CLAUDE.md](../../../CLAUDE.md) («`dropout` son C»), [organizacion.md](../../../docs/organizacion.md) §H («reservar Optuna para lo continuo: `lr`, canales, dropout») e [instructionsNewNN.md](../../../instructionsNewNN.md) §9 — pero `NETWORK_DEFAULTS` no tiene el campo y el `forward` de [builder.py](../../../src/fv/models/builder.py) no aplica ninguna capa de dropout. **Poner `dropout` como eje de un recorrido hoy no haría nada** (`full_config` filtra por `NETWORK_DEFAULTS`, así que el campo se descartaría en silencio y los N puntos entrenarían la misma red). Antes de barrerlo hay que **implementarlo en C**. Motivo medido para hacerlo: ver la nota ⚠ de abajo. |
...
| **10 quater** | **`dropout`** ∈ {0, 0.1, 0.25}, 5 semillas | 15 runs · ≈0,8 $ **+ implementarlo** | Misma diana que el anterior (el sobreajuste medido), pero **exige trabajo de código antes**: hoy `dropout` está en la documentación y **no en la red** (ver §1). Orden recomendado: (a) correr `weight_decay`, que es gratis de preparar; (b) si la regularización mueve la aguja, implementar `dropout` en C y barrerlo; (c) si no la mueve, **no implementarlo** y borrar la mención de los documentos, que hoy promete un mando que no existe. ⚠ Al implementarlo hay que verificar que `dropout: 0.0` deja los checkpoints existentes cargando y dando **la misma salida bit a bit** — el proyecto ya tiene ese test para la reparametrización de la geometría. |
...
- **Documentado pero NO implementado:** **`dropout`** — tres documentos lo nombran como parámetro de
